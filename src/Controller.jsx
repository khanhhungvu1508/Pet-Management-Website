...
export default class Controller extends React.Component {
    ...
    removeProduct = (index) => {
        ...
        this.setState((prevState) => ({
            totalPrice: parseFloat((prevState.totalPrice - mycartDelete.price * mycartDelete.quantity).toFixed(2)),
            myCart: mycarts
        }));
    }
    ...
    updateQuantity = (index, mode) => {
        ...
        mycart.quantity = (mode === -1)? Math.max(prevQuantity - 1, 1): Math.min(prevQuantity + 1, 99);
        // Update totalPrice = prevTotalPrice + priceProduct * (quantity - prevQuantity)
        this.setState((prevState) => ({
            totalPrice: parseFloat((prevState.totalPrice + priceProduct * (mycart.quantity - prevQuantity)).toFixed(2))
        }));
    ...
    componentDidMount = () => {
        ...
        this.changeZImage = setInterval(() => {
            ...
            this.setState((prevState) => ({
                rotateZImage: (prevState.rotateZImage + rotateAngle > 360)
                                ? prevState.rotateZImage + rotateAngle - 360: prevState.rotateZImage + rotateAngle
            }));
        }, 100);
    }
